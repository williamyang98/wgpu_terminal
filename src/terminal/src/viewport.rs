...
pub struct Viewport {
    ...
    pub(crate) scrollback_buffer: Option<ScrollbackBuffer>, // eject lines into scrollback buffer
    pub(crate) pen: Pen,
...
impl Viewport {
    ...
    fn eject_oldest_line_into_scrollbuffer(&mut self) {
        ...
        if let Some(ref mut scrollback_buffer) = self.scrollback_buffer {
            scrollback_buffer.extend_current_line(&line[..line_status.length]);
            if line_status.is_linebreak {
                scrollback_buffer.advance_line();
            }
        }
